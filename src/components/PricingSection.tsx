...
function PricingTier({
  ...
  return <div className={`group relative rounded-2xl overflow-hidden opacity-0 animate-[fadeInUp_0.8s_ease-out_forwards] transition-all duration-300 border ${highlighted ? 'bg-white/10 border-white/20 scale-105 shadow-lg' : 'bg-white/10 border-white/20 hover:shadow-md'}`} style={{
    animationDelay: delay
  }}>
      {/* Icon Badge */}
      <div className={`absolute top-6 right-6 w-12 h-12 rounded-full bg-white flex items-center justify-center text-[#2d4a4a] transition-all duration-300`}>
        {icon}
      </div>
      ...
      <div className="p-8">
        ...
        {/* CTA */}
        <Button className="w-full bg-white text-[#2d4a4a] hover:bg-white/90 border-0">
          Enroll Now
        </Button>
      </div>
    </div>;
  ...
  return <AnimatedSection className="py-20 block bg-[#2d4a4a]" animation="fade-in-up">
      <div className="max-w-[1400px] mx-auto px-6 md:px-12 lg:px-20">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="text-xs tracking-wide uppercase text-white/70 font-semibold mb-4 block">
